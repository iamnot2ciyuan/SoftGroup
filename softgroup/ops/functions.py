...
def _get_ops():
    ...
    if _ops_module is None:
        ...
        # 优先尝试SOFTGROUP_OP（新编译的Python 3.9版本）
        ops_path = os.path.join(ops_dir, 'SOFTGROUP_OP.cpython-39-x86_64-linux-gnu.so')
        module_name = "SOFTGROUP_OP"  # 模块名必须与.so文件名匹配
        if os.path.exists(ops_path):
            spec = importlib.util.spec_from_file_location(f"softgroup.ops.{module_name}", ops_path)
            _ops_module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(_ops_module)
        else:
            # 尝试Python 3.9版本的ops
            ops_path = os.path.join(ops_dir, 'ops.cpython-39-x86_64-linux-gnu.so')
            module_name = "ops"
            if os.path.exists(ops_path):
                spec = importlib.util.spec_from_file_location(f"softgroup.ops.{module_name}", ops_path)
                _ops_module = importlib.util.module_from_spec(spec)
                ...
                # 尝试Python 3.8版本（向后兼容）
                ops_path = os.path.join(ops_dir, 'ops.cpython-38-x86_64-linux-gnu.so')
                if os.path.exists(ops_path):
                    spec = importlib.util.spec_from_file_location(f"softgroup.ops.{module_name}", ops_path)
                    _ops_module = importlib.util.module_from_spec(spec)
                    spec.loader.exec_module(_ops_module)
                    ...
                    # 如果找不到.so文件，尝试正常导入
                    try:
                        from . import SOFTGROUP_OP as _ops
                        _ops_module = _ops
                    except ImportError:
                        try:
                            ...
                            raise ImportError(f"无法加载softgroup.ops模块，找不到.so文件在: {ops_dir}")
    return _ops_module
...
def octree_ball_query(coords, mean_active, radius):
    ...
    _get_ops().build_and_export_octree(coords, xyzwhl, boxes, pt_inds, pt_start_len, num_levels)
    boxes = boxes.cuda()
...
class GetMaskIoUOnCluster(Function):
    ...
    def forward(ctx, proposals_idx, proposals_offset, instance_labels, instance_pointnum):
        ...
        _get_ops().get_mask_iou_on_cluster(proposals_idx, proposals_offset, instance_labels,
...
class GetMaskIoUOnPred(Function):
    ...
    def forward(ctx, proposals_idx, proposals_offset, instance_labels, instance_pointnum,
        ...
        _get_ops().get_mask_iou_on_pred(proposals_idx, proposals_offset, instance_labels,
                                 instance_pointnum, proposals_iou, nInstance, nProposal,
                                 mask_scores_sigmoid)
...
class GetMaskLabel(Function):
    ...
    def forward(ctx, proposals_idx, proposals_offset, instance_labels, instance_cls,
        ...
        _get_ops().get_mask_label(proposals_idx, proposals_offset, instance_labels, instance_cls,
...
class Voxelization_Idx(Function):
    ...
    def forward(ctx, coords, batchsize, mode=4):
        ...
        _get_ops().voxelize_idx(coords, output_coords, input_map, output_map, batchsize, mode)
...
class Voxelization(Function):
    ...
    def forward(ctx, feats, map_rule, mode=4):
        ...
        _get_ops().voxelize_fp(feats, output_feats, map_rule, mode, M, maxActive, C)
        return output_feats
    ...
    def backward(ctx, d_output_feats):
        ...
        _get_ops().voxelize_bp(d_output_feats.contiguous(), d_feats, map_rule, mode, M, maxActive, C)
...
class BallQueryBatchP(Function):
    ...
    def forward(ctx, coords, batch_idxs, batch_offsets, radius, meanActive):
        ...
        while True:
            ...
            nActive = _get_ops().ballquery_batch_p(coords, batch_idxs, batch_offsets, idx, start_len, n,
                                            meanActive, radius)
...
class BFSCluster(Function):
    ...
    def forward(ctx, cluster_numpoint_mean, ball_query_idxs, start_len, threshold, class_id):
        ...
        _get_ops().bfs_cluster(cluster_numpoint_mean, ball_query_idxs, start_len, cluster_idxs,
...
class GlobalAvgPool(Function):
    ...
    def forward(ctx, feats, proposals_offset):
        ...
        _get_ops().global_avg_pool_fp(feats, proposals_offset, output_feats, nProposal, C)
    ...
    def backward(ctx, d_output_feats):
        ...
        _get_ops().global_avg_pool_bp(d_feats, proposals_offset, d_output_feats.contiguous(), nProposal, C)
...
class SecMean(Function):
    ...
    def forward(ctx, inp, offsets):
        ...
        _get_ops().sec_mean(inp, offsets, out, nProposal, C)
...
class SecMin(Function):
    ...
    def forward(ctx, inp, offsets):
        ...
        _get_ops().sec_min(inp, offsets, out, nProposal, C)
...
class SecMax(Function):
    ...
    def forward(ctx, inp, offsets):
        ...
        _get_ops().sec_max(inp, offsets, out, nProposal, C)
